...
class TaskItem:
    ...
    def to_dict(self, status, completed_at):
        """转换为字典用于日志记录"""
        return {
            "id": self.task_id,
            "name": self.name,
            ...
            "status": status,  # completed 或 skipped
...
class Logger:
    ...
    def log_task(self, task, status):
        ...
        # 添加任务记录
        task_record = task.to_dict(status, completed_at)
    ...
    def get_month_statistics(self, year, month):
        ...
        completed_tasks = 0
        skipped_tasks = 0
        total_focus_minutes = 0  # 总专注时长
        total_break_minutes = 0  # 总休息时长
        completed_focus_minutes = 0  # 已完成任务专注时长
        ...
        # 计算平均数
        avg_daily_tasks = total_tasks / total_days if total_days > 0 else 0
    ...
    def export_month_to_csv(self, year, month, file_path):
        ...
        lines = []
        lines.append("日期,任务ID,任务名称,预估时长(分钟),休息次数,休息时长(分钟),状态,完成时间")
        
        for date_str in sorted(logs.keys()):
            log = logs[date_str]
            for task in log["tasks"]:
                status_text = "已完成" if task["status"] == "completed" else "已跳过"
                lines.append(f'{date_str},{task["id"]},"{task["name"]}",{task["duration"]},{task["break_count"]},{task["break_duration"]},{status_text},{task["completed_at"]}')
...
class TimerApp:
    ...
    def _create_task_queue_area(self):
        ...
        # 列宽定义（像素）- 统一表头和数据行
        self.col_widths = [80, 300, 120, 120, 120, 50]  # 序号、任务名称、预估时长、休息次数、休息时长、删除
        ...
        headers = ["序号", "任务名称", "预估时长(分钟)", "休息次数", "休息时长(分钟)", ""]
    ...
    def _add_new_task(self, task_name=None):
        ...
        # 序号（固定文本）
        task.seq_label = tk.Label(self.table_frame, text=f"任务{task.task_id}",
                                  font=("Microsoft YaHei", 9), bg="#f0f0f0", fg="#333333")
        task.seq_label.grid(row=row_idx, column=0, padx=2, pady=2, sticky="w")
    ...
    def _highlight_current_task(self):
        ...
        for i, task in enumerate(self.tasks):
            ...
            if i == self.current_task_index and self.is_running and not self.is_paused:
                # 当前任务：深绿色高亮框
                for col, widget in widgets_by_col.items():
                    if isinstance(widget, tk.Entry):
                        ...
                        if col == 0:  # 序号列
                            widget.config(text=f"任务{task.task_id}", bg="#BBF7D0", fg="#166534")
                        else:
                            widget.config(bg="#BBF7D0", fg="#166534")
                ...
                for col, widget in widgets_by_col.items():
                    if isinstance(widget, tk.Entry):
                        ...
                        widget.config(text=f"✓ 任务{task.task_id}", bg="#DCFCE7", fg="#166534")
                    elif isinstance(widget, tk.Button):
                        widget.config(bg="#BBF7D0", fg="#166534")
            else:
                # 普通任务：灰色
                for col, widget in widgets_by_col.items():
                    if isinstance(widget, tk.Entry):
                        ...
                        widget.config(text=f"任务{task.task_id}", bg="#f0f0f0", fg="#333333")
                    elif isinstance(widget, tk.Button):
    ...
    def _start_queue(self):
        ...
        # 初始化状态（如果current_task_index已经设置过，则从该位置继续）
    ...
    def _task_completed(self):
        ...
        # 记录日志
        self.logger.log_task(task, "completed")
...
'''
    ...
    def _show_calendar_window(self):
        ...
        def _render_calendar(year, month):
            ...
            for day in range(1, days_in_month + 1):
                ...
                if date_str in logs:
                    ...
                    completed_tasks = sum(1 for t in log["tasks"] if t["status"] == "completed")
        ...
        def _update_statistics(year, month):
            ...
            tk.Label(row2, text=f"⏱️ 总专注时长：{stats['total_focus_minutes']}分钟", 
                    font=("Microsoft YaHei", 10), bg="#f0f0f0").pack(side=tk.LEFT, padx=5)
